app.views.list = Backbone.View.extend({
  ...
  render: function() {
    ...
    this.model.each(function(todo, index) {
      if (self.mode === 'archive' ? todo.get('archived') === true :
        todo.get('archived') === false) {
        var template = Handlebars.compile($('#tmpl-list-item').html());
        html += template({
  ...
  priorityUp: function(e) {
    var index =
    parseInt(e.target.parentNode.parentNode.getAttribute('data-index'));
    this.model.up(index);
  },
